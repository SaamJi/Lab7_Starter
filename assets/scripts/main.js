...
async function init() {
  ...
  let recipes;
  try {
    console.log("init() try block running");
 ...
 * @returns {Promise<Array<Object>>} {Array<Object>} An array of recipes found in localStorage
 */
async function getRecipes() {
  ...
  if (recipes !== null && recipes.length !== 0) {
    console.log("Found recipes in local storage.");
    return recipes;
  }

  console.log("Did not found recipes in local storage.");
  /**************************/
  ...
  return new Promise((resolve, reject) => {
    ...
    //            declared above
    RECIPE_URLS.forEach(async function (element, index) {
      ...
      try {
        ...
        const myRequest = new Request(element);
        ...
        //            resolve() method.
        console.log(index);
        console.log(RECIPE_URLS.length);
